...
@ManagedBean(name = T102Controller.CONTROLLER_NAME)
@SessionScoped
public class T102Controller extends BaseController {

    private static final Logger LOG = Logger.getLogger(T102Controller.class);
    public static final String CONTROLLER_NAME = "t102Controller";
        ...
        /*    if("ADMIN".equals(userInfo.getAdminUser().getRoleId().getRoleName())){
             ...
             */
             ...
             if (null == selected.getStatus()) {
                 JsfUtil.addFacesErrorMessage(MessageBundleLoader.getMessageFormat("messages.code.2002", "สถานนะ"));
                 RequestContext.getCurrentInstance().scrollTo("listForm:create_msg");
                 return;
             }
             ...
             for (SysPrepareTransportDetail sysPrepareTransportDetail_ : prepareDetail) {
                 ...
                 sysManufactoryReal.setStatus(Constants.TRANSPORTER_NORMAL);
          ...
          /*  if("ADMIN".equals(userInfo.getAdminUser().getRoleId().getRoleName())){
            ...
            //update manufactory_real status on_process
             for (SysPrepareTransportDetail sysPrepareTransportDetail_ : selected.getSysPrepareTransportDetailList()) {
                 //manufactoryFacade.updateStatusTransporterSysManufactoryReal(Constants.TRANSPORTER_PREPARE,sysPrepareTransportDetail_.getFactoryRealId());
                 SysManufactoryReal sysManufactoryReal = sysPrepareTransportDetail_.getFactoryRealId();
                 sysManufactoryReal.setStatusTransporter(Constants.TRANSPORTER_PREPARE);
                 manufactoryFacade.editSysManufactoryReal(sysManufactoryReal);
             }
        ...
        try {
            //update manufactory_real status on_process
             for(SysPrepareTransportDetail sysPrepareTransportDetail_:selected.getSysPrepareTransportDetailList()){
                 //manufactoryFacade.updateStatusTransporterSysManufactoryReal(Constants.TRANSPORTER_NORMAL,sysPrepareTransportDetail_.getFactoryRealId());
                 SysManufactoryReal sysManufactoryReal = sysPrepareTransportDetail_.getFactoryRealId();
                 sysManufactoryReal.setStatusTransporter(Constants.TRANSPORTER_NORMAL);
                 manufactoryFacade.editSysManufactoryReal(sysManufactoryReal);
             }
    ...
    public void search() {
        try {
              ...
              items=transporterFacade.findSysPrepareTransportListByCriteria(foreman_find,documentno,workunit_find,status_find, startDate, toDate);
     ...
     public void searchManufactory(){
         try {
             manufactoryReal_items_dialog = manufactoryFacade.findTranSporterSysManufactoryRealListByCriteria(null!=manufactory_selected?manufactory_selected.getDocumentno():null,
                     selected.getForemanId(), selected.getWorkunitId(), null, null, Constants.TRANSPORTER_NORMAL);
         } catch (Exception ex) {
             LOG.error(ex);
        }
    }  
