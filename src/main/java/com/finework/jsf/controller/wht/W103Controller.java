...
@Named(W103Controller.CONTROLLER_NAME)
@SessionScoped
public class W103Controller extends BaseController {

    private static final Logger LOG = Logger.getLogger(W103Controller.class);
    public static final String CONTROLLER_NAME = "w103Controller";
...
//        runningNoCustomer();
    ...
    public void create() {
      try {
            ...
            selected.setVatType(Constants.VAT5_PERSEN);
    ...
    public void reportPDF() {
        try {
            ...
            map.put("reportCode", "W103");
            report.exportWHT("W103","50TV", "WHT", map,reportList_);
        } catch (Exception ex) {
            JsfUtil.addFacesErrorMessage(MessageBundleLoader.getMessage("messages.code.9001"));
            LOG.error(ex);
        }
    ...
    public void search() {
        try {
              ...
              items=whtFacade.findSysWhtListByCriteriat(documentNo,cust_find,startDate,toDate,Constants.VAT5_PERSEN);
   ...
   //===== start  Dialog=========  
    public void addDetail(){
         try {
            ...
            if (selected.getSysWhtDetailList() == null) 
             ...
             whtDetail_selected.setAmountVat(whtDetail_selected.getAmount()*0.05);
    ...
    public Double checkTotalVat(List<SysWhtDetail> sysBLDetail){
        ...
        if(null !=sysBLDetail && sysBLDetail.size()>0){
            ...
            this.totalVatDetail=(totalDetail_==0.0)?0.0:(totalDetail_*0.05);
