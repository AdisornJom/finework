...
@Named(B109Controller.CONTROLLER_NAME)
@SessionScoped
public class B109Controller extends BaseController {

    private static final Logger LOG = Logger.getLogger(B109Controller.class);
    public static final String CONTROLLER_NAME = "b109Controller";
    ...
    public void create() {
      try {
            ...
            if (null==selected.getBillingDate()) {
                JsfUtil.addFacesErrorMessage(MessageBundleLoader.getMessageFormat("messages.code.2002", "วันที่วางบิล"));
                RequestContext.getCurrentInstance().scrollTo("listForm:create_msg");
                return;
            }
            
            if (null==selected.getProductSendDate()){
            ...
            //insert Billinge
            selected.setBillingType(Constants.BILLING_B109);
...
//            }
    ...
    public void printPdfMuti(){
         try {
             ...
             for (SysBilling sysBilling : printSelected) {
                    ...
                    map.put("billingdate",DateTimeUtil.cvtDateForShow(rpt_sysbilling.getBillingDate(), "dd/MM/yyyy", new Locale("th", "TH")));

                    map.put("product_send_date",null!=rpt_sysbilling.getProductSendDate()?DateTimeUtil.cvtDateForShow(rpt_sysbilling.getProductSendDate(), "dd/MM/yyyy", new Locale("th", "TH")):"-");
                   ...
                   // map.put("bill_date",DateTimeUtil.cvtDateForShow(rpt_sysDelivery.getBillDateLast(), "dd/MM/yyyy", new Locale("th", "TH")));
                    ...
                    map.put("reportCode", "B109");
                    JasperPrint print= report.exportSubReport_Template_mearge("template.jpg","b109", new String[]{"B109Report","B109SubReport"}, "B109", map, reportList_);
    ...
    public void search() {
        try {
             ...
             items=billingFacade.findSysBillingListByCriteria(Constants.BILLING_B109,documentno,companyName,startDate,toDate);
//             lazyBillingModel = new LazyBillingDataModel(billingFacade,Constants.BILLING_B109,documentno,StringUtils.trimToEmpty(companyName),startDate,toDate);
