...
@Named(B107Controller.CONTROLLER_NAME)
@SessionScoped
public class B107Controller extends BaseController {

    private static final Logger LOG = Logger.getLogger(B107Controller.class);
    public static final String CONTROLLER_NAME = "b107Controller";
...
//            }
            ...
            //insert Billinge
            selected.setBillingType(Constants.BILLING_B107);
...
//            }
    ...
    public void reportPDF() {
        try {
           ...
           // map.put("bill_date",DateTimeUtil.cvtDateForShow(rpt_sysDelivery.getBillDateLast(), "dd/MM/yyyy", new Locale("th", "TH")));
            ...
            report.exportSubReport_Heading("delivery_bill.png","b107", new String[]{"B107Report","B107Report"}, "B107", map, reportList_); 
        } catch (Exception ex) {
            JsfUtil.addFacesErrorMessage(MessageBundleLoader.getMessage("messages.code.9001"));
            LOG.error(ex);
        }
         ...
         try {
             ...
             for (SysBilling sysBilling : printSelected) {
                   ...
                   // map.put("bill_date",DateTimeUtil.cvtDateForShow(rpt_sysbilling.getBillDateLast(), "dd/MM/yyyy", new Locale("th", "TH")));
                    ...
                    map.put("reportCode", "B107");
                    JasperPrint print= report.exportSubReport_Template_mearge_heading("delivery_bill.png","b107", new String[]{"B107Report","B107SubReport"}, "B107", map, reportList_);
    ...
    public void search() {
        try {
             ...
             items=billingFacade.findSysBillingListByCriteria(Constants.BILLING_B107,documentno,StringUtils.trimToEmpty(companyName),startDate,toDate);
            // lazyBillingModel = new LazyBillingDataModel(billingFacade,Constants.BILLING_B107,documentno,StringUtils.trimToEmpty(companyName),startDate,toDate);
         ...
         try {
             //validate field iteam 
              if (detail_selected==null
                    //||StringUtils.isBlank(detail_selected.getDetailDesc())
                    ||StringUtils.isBlank(dvDetail_selected.getBillConvert())
                    ||StringUtils.isBlank(dvDetail_selected.getHouseNo())
                    ||StringUtils.isBlank(dvDetail_selected.getUnit())
...
//            }
            ...
            if (selected.getSysBillingDetailList() == null) 
             ...
             dvDetail_selected.setDetailCode(detail_selected.getDetailCode());
             ...
             for(SysBillingDetail sysBillingDetail:selected.getSysBillingDetailList()){
                 listHouse.add(sysBillingDetail.getHouseNo());
                 listConvert.add(sysBillingDetail.getBillConvert());
             }
             if (!listHouse.contains(dvDetail_selected.getHouseNo()) && !listConvert.contains(dvDetail_selected.getBillConvert())) {
