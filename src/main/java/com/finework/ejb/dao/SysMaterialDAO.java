...
public class SysMaterialDAO extends AbstractDAO<SysMaterial>
{
  ...
  {
    Query q = this.em.createQuery("select o from SysMaterial o where o.status ='Y' order by  materialDesc asc");
    return q.getResultList();
  }

  public SysMaterial findSysMaterialById(Integer detailId) {
    Query q = this.em.createQuery("select o from SysMaterial o where o.status ='Y' and o.materialId =:materialId  order by  materialDesc asc");
  ...
  public List<ReportStockI105TO> findReportSummaryI105(SysMaterial material, SysContractor contractor, int[] range) throws Exception {
    StringBuilder sb = new StringBuilder();
    sb.append("select  material_id,contractor_id,material_desc,contractor_name_th,contractor_nickname, \tsum(income) as income,sum(recive) recive,sum(return_) returnstock,sum(repair_in) repair_in,\tsum(outcome) outcome,sum(take) take,sum(borrow) borrow,sum(repair_out) repair_out,\tsum(income)-sum(outcome) as total from (  SELECT g.material_id,f.contractor_id,g.material_desc, f.contractor_name_th,f.contractor_nickname,sum(c.quantity) as income,         sum(CASE WHEN c.status = 'P' THEN c.quantity ELSE 0 END) AS recive,         sum(CASE WHEN c.status = 'R' THEN c.quantity ELSE 0 END) AS return_,         sum(CASE WHEN c.status = 'Y' THEN c.quantity ELSE 0 END) AS repair_in,         0 outcome,0 take,0 borrow,0 repair_out    FROM sys_material_receipts c,sys_contractor f,sys_material g     where c.contractor_id=f.contractor_id    and c.material_id=g.material_id");
    ...
    sb.append(" GROUP BY g.material_desc,f.contractor_name_th,f.contractor_nickname  union ALL  SELECT g.material_id,f.contractor_id,g.material_desc,f.contractor_name_th,f.contractor_nickname,0 income,   0 recive,0 return_,0 repair_in,sum(b.quantity) as outcome,   sum(CASE WHEN a.status = 'Y' THEN b.quantity ELSE 0 END) AS take,   sum(CASE WHEN a.status = 'B' THEN b.quantity ELSE 0 END) AS borrow,   sum(CASE WHEN a.status = 'P' THEN b.quantity ELSE 0 END) AS repair_out  FROM sys_material_expenses a,sys_material_expenses_detail b,sys_contractor f,sys_material g   where a.contractor_id=f.contractor_id\tand b.material_id=g.material_id\tand a.expenses_id=b.expenses_id ");
    ...
    Query q = this.em.createNativeQuery(sb.toString(), ReportStockI105TO.class);
    q.setMaxResults(range[1] - range[0]);
    q.setFirstResult(range[0]);
    return q.getResultList();
  }
