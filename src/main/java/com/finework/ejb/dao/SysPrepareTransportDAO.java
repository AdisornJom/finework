...
public class SysPrepareTransportDAO extends AbstractDAO<SysPrepareTransport> {
    ...
    public List<SysPrepareTransport> findSysPrepareTransportList() {
        Query q = em.createQuery("select o from SysPrepareTransport o where o.status ='Y' order by  o.tpserviceDesc asc");
        return q.getResultList();
    }
    
    public SysPrepareTransport findSysPrepareTransportById(Integer prepareTpId) {
        Query q = em.createQuery("select o from SysPrepareTransport o where o.prepareTpId =:prepareTpId  order by o.prepareTpId asc");
    ...
    public void updateStatusSysPrepareTransportByprepareTpId(Integer status,Integer prepareTpId){
        Query q = em.createQuery("update SysPrepareTransport o set o.status=:status where o.prepareTpId=:prepareTpId");
